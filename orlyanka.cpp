...
public:
    void registration()
    {
        ...
        for (int num = 0; num < count_of_players; num++)
        {
            ...
            cout << "Weclome to  " << name << " "
                 << "Your balance is " << new_player.get_money() << " "
                 << "gold" << endl;
            cout << endl;
    ...
    void new_game()
    {
        if (count_of_players > 0)
        {
            ...
            for (int i = 0; i < count_of_players; i++)
            {
                ...
                cout << "Player " << list_of_players[i].get_name() << " "
                     << "makes a bet 5 gold" << endl;
            }
            cout << endl
                 << "Base installation" << endl;
            cout << "The base is set. Coordinates: " << base.get_coordinates() << ". "
                 << "Width of base: " << base.get_width() << endl;
            click();
        }
        else
        {
    ...
    void continuation_of_the_game()
    {
        for (int i = 0; i < count_of_players; i++)
        {
            if (list_of_players[i].get_money() <= 0)
            {
                cout << "Goodbye " << list_of_players[i].get_name() << endl;
                list_of_players.erase(list_of_players.begin() + i);
                count_of_players--;
            }
            cout << list_of_players[i].get_name() << " "
                 << "If you want to continue type yes" << endl;
            string question;
            cin >> question;
            if (question == "yes")
            {
